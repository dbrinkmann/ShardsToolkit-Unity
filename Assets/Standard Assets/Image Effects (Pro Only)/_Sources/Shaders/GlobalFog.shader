...
CGINCLUDE
	...
	float ComputeFogForYAndDistance (in float3 camDir, in float3 wsPos) 
	{
		...
		fogVert *= fogVert; 		
		return  max((1-exp(-_GlobalDensity*fogInt)), exp (-fogVert));
	}
	
	float ComputeFogForYOrDistance (in float3 camDir, in float3 wsPos) 
	{
		float fogInt = saturate(length(camDir) * _StartDistance.x-1.0) * _StartDistance.y;	
		float fogVert = max(0.0, (wsPos.y-_Y.x) * _Y.y);
		fogVert *= fogVert; 
	...
	half4 fragRelativeYOrDistance (v2f i) : COLOR
	{
		float dpth = Linear01Depth(UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture,i.uv_depth)));
		float4 wsDir = dpth * i.interpolatedRay;
		return lerp(tex2D(_MainTex, i.uv), _FogColor, ComputeFogForYOrDistance(wsDir.xyz, wsDir.xyz));
	}

	half4 fragRelativeYOrDistance2(v2f i) : COLOR
	{
		float dpth = Linear01Depth(UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture,i.uv_depth)));
		clip(0.9999 - dpth);
		half4 texColor = tex2D(_MainTex, i.uv);	
		clip(texColor.r + texColor.g + texColor.b - 0.0001);
		float4 wsDir = dpth * i.interpolatedRay;
		return lerp(texColor, _FogColor, ComputeFogForYOrDistance(wsDir.xyz, wsDir.xyz));
	}
...
SubShader {
	...
	Pass {
		...
		#pragma vertex vert
		#pragma fragment fragRelativeYOrDistance
		#pragma fragmentoption ARB_precision_hint_fastest 
		#pragma exclude_renderers flash
	...
	Pass {
		...
		#pragma vertex vert
		#pragma fragment fragRelativeYOrDistance2
		#pragma fragmentoption ARB_precision_hint_fastest 
		#pragma exclude_renderers flash
